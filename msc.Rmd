...
barplot_MDC_single_slope <- 
  ...
  scale_y_continuous(limits = c(0, 10), 
                     labels = label_number(scale = 1, accuracy = 0.1)) +
...
calculateMDCWithAdditionalSamples <- function(model, additional_samples, alpha, power) {
  ...
  MDC <- (t_alpha + t_beta) * SE_b * 365
  
  return(MDC)
}
```

Now I tried the function for station 12301, which had the largest MDC
...
MDC_cur <- calculateMDCWithAdditionalSamples(model_cur,0, 
                                         alpha = 0.05, power = 0.9)

MDC_add <- calculateMDCWithAdditionalSamples(model_cur,additional_samples, 
                                         alpha = 0.05, power = 0.9)

print(paste("MDC for", initial_n + additional_samples, "total samples:", 
            sprintf("%.2f", abs(10^(MDC_add) - 1) * 100), "% compared to",
            sprintf("%.2f", abs(10^(MDC_cur) - 1) * 100),
            "% for", initial_n, "initial samples"))


```

I need a way to incorporate the x data into the regression statistics and MDC. I think the approach as is assumes that each additional sample is collected at a constant step in x (or time in my case).
